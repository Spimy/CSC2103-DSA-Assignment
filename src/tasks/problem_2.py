...
class MinHeap: 
      ...
      def insert(self, item: Tuple[int, str]) -> None: 
            # Insert item while maintaining the heap property.
            self.heap.append(item)
            self.size += 1
            self._heapify_up(self.size - 1)

      def extract_min(self) -> Optional[Tuple[int, str]]:
...
class DijkstraAlgorithm():
      def solve(self, graph: Dict[str, List[Tuple[str, int]]], source: str) -> Dict[str, int] | None:
      ...
      def print_distances(self, distances: Dict[str, int], source: str) -> None:
      ...
      def _is_valid_graph(self, graph: Dict[str, List[Tuple[str, int]]], source: str) -> bool:
            ...
            for vertex, edges in graph.items():
                  for destination, weight in edges: 
                        if weight < 0:
                              print(f"Negative weight found: {weight} for edge {vertex} -> {destination}.")
                              return False
      ...
      def _dijkstra(self, graph: Dict[str, List[Tuple[str, int]]], source: str) -> Dict[str, int]:
            ...
            while not priority_queue.is_empty():
                  current_dist, current_vertex = priority_queue.extract_min() 
      ...
      def get_path(self, graph: Dict[str, List[Tuple[str, int]]], source: str, destination: str) -> List[str] | None:  
